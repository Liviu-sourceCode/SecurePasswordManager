...
echo "Tauri lockfile entry:"
grep -nA1 'name = "tauri"' src-tauri/Cargo.lock | head -2

if ! grep -nA1 'name = "tauri"' src-tauri/Cargo.lock | grep -q 'version = "2.11.3"'; then
  echo "ERROR: expected tauri 2.11.3 in Cargo.lock"
  exit 1
fi
'''
      }
